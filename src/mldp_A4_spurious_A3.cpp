...
class Path{
    public:
        ...
        int& spurious_num(){return m_spurious_num;}
    private:
...
void update_likelihood(int k,double pt,Path& P){
    ...
    if(MODULATION == "jitter"){
        ...
        P.likelihood() = res+P.spurious_num()*beta_;
        return;
    }
...
void init_paths(){
    // for(int i = 0;i<K;++i){
    paths.emplace_back(forward<Path>(Path(K,known,0,toa[0])));
    // }
...
int main(int argc,char * argv[]){
    ...
    for(int i = 1;i<toa.size();++i){
        ...
        for(auto P:paths){
            // for(int k = 0;k<=K;++k){
                auto __P = P;
                __P.path().push_back(known); // 更新路径
                // 1.如果该路径曾经分配过脉冲给源k，即可以计算似然函数
                // 计算杂散脉冲的似然值
                if(__P.last_pulse_idx()[known]!=-1){
                    double pt = toa[i]-toa[__P.last_pulse_idx()[known]];
                    update_likelihood(known,pt,__P);
                }
                else{
                    update_likelihood(known,toa[i],__P);
                }

                // 2.更新最后脉冲位置
                __P.last_pulse_idx()[known] = i;
    ...
    int choose_known = 0;
    int unknown_pulse = 0;
    int choose_unknown = 0;
    auto path = results.path();
    for(int i = 0;i<path.size();++i){
        if(label[i] == known){
            known_pulse += 1;
            choose_known += path[i] == known? 1:0;
        }
        else{
            unknown_pulse += 1;
            choose_unknown += path[i] == known? 1:0;
        }
        // if(path[i]!=label[i]){
        //     cnt_wrong += 1;
    ...
    // LOG_INFO << "algorithm: spurious_modified_A4";
    LOG_INFO << MODULATION.c_str() << "  "<< TYPE.c_str() << RATE.c_str();
    LOG_INFO << "known_pulse:" << "  "<< known_pulse ;
    LOG_INFO << "choose_known:" <<"  "<< choose_known;
    LOG_INFO << "unknown_pulse: " << unknown_pulse;
    LOG_INFO << "choose_unknown: " << choose_unknown;
    // LOG_INFO << "accuracy: "<< 1-1.0*cnt_wrong/path.size() ;
    // LOG_INFO << RATE.c_str() << " " << 1-1.0*cnt_wrong/path.size();
    // LOG_INFO << beta_ << " " << 1-1.0*cnt_wrong/path.size();
